...
public:
    //опр метода, который будет вызываться когда свойство объекта изменится. 
...
//интрфейс для наблюдаемого объекта 
class INotifyDataChanged {
public:
...
public:
    virtual bool on_property_changing(INotifyDataChanging* object, const std::string& property_name,
...
public:
    ...
    void set_username(const std::string& new_name) {
        ...
        username = new_name;
        //Слушатели получают информацию об изменении username
